...
def detect_hardware() -> List[str]:
    ...
    # Intel RealSense
    try:
        result = subprocess.run(["rs-enumerate-devices"], capture_output=True, text=True, timeout=3)
    ...
    # Zed camera
    try:
        result = subprocess.run(["ZEDMiniDetection"], capture_output=True, text=True, timeout=3)
