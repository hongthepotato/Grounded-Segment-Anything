...
class YoloInferenceNode(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info(
            "YoloInferenceNode ready. input=%s output=%s conf=%.2f", input_topic, output_topic, self.conf
        )
    ...
    def _load_model(self):
        ...
        if enable_trt:
            try:
                self.get_logger().info("First boot: converting to TensorRT (~2 min)...")
                ...
                self.get_logger().warn("TensorRT export failed, falling back to PyTorch: %s", str(e))
    ...
    def on_image(self, msg: Image):
        ...
        avg_fps = (
            self._frame_count / (self._total_inference_ms / 1000.0) if self._total_inference_ms > 0 else 0.0
        )
