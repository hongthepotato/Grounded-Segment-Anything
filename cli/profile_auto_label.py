...
# E402 suppressed: import after the sys.path inserts above.
...
def profile_sequential_detailed(
    image_paths: List[str], class_prompts: List[str], config: Dict[str, Any], device: str
) -> ProfileResult:
    """
    ...
    for image_path in image_paths:
        ...
        # Phrase extraction. Result intentionally unused — this is a profiler
        # measuring the cost of the comprehension itself; renamed `_phrases`
        # to silence F841 without removing the work.
        _phrases = [
        ...
        if len(boxes_scaled) > 0:
            nms_idx = torchvision.ops.nms(boxes_scaled, logits.max(dim=1)[0], config["nms_threshold"])
    ...
    return ProfileResult(
        mode="sequential", batch_size=1, timing=timing, gpu_memory_peak_mb=get_gpu_memory_mb()
    )


def profile_batched_detailed(
    image_paths: List[str], class_prompts: List[str], batch_size: int, config: Dict[str, Any], device: str
) -> ProfileResult:
    """
    ...
    for batch_start in range(0, len(image_paths), batch_size):
        ...
        for b in range(actual_batch_size):
            ...
            # Phrase extraction (intentionally unused — profiler measures the
            # comprehension cost itself; renamed `_phrases` to silence F841).
            _phrases = [
                get_phrases_from_posmap(logit > config["text_threshold"], tokenized, tokenizer).replace(
                    ".", ""
                )
...
def print_timing_table(results: List[ProfileResult]):
    ...
    print("=" * 120)
    print(
        f"{'Mode':<12} {'Batch':<6} {'Images':<7} {'I/O':<8} {'Preproc':<8} "
    ...
    for r in results:
        ...
        print(
            f"{r.mode:<12} {r.batch_size:<6} {t.num_images:<7} "
...
def main():
    ...
    if args.compare_sequential:
        ...
        logger.info(
            f"  Total: {result.timing.total:.3f}s ({result.timing.total / result.timing.num_images:.4f}s/img)"
        )
