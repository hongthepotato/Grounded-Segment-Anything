...
class _FakeNode:
    def __init__(self, name): self._logger = MagicMock()
...
class _FakeDetection:
    def __init__(self, class_id="defect", score=0.9, size_x=50.0, size_y=50.0):
        ...
        self.bbox = MagicMock(center=MagicMock(position=MagicMock(x=100.0, y=100.0)), size_x=size_x, size_y=size_y)
...
class TestDefectFlagging:
    def _make_node(self, monkeypatch, **env_vars):
        ...
        spec = importlib.util.spec_from_file_location(
            "defect_flagging_node",
            repo_root / "blocks" / "defect-flagging" / "node.py"
        )
...
class TestConfidenceFilter:
    def _make_node(self, monkeypatch, min_confidence="0.7"):
        ...
        import importlib.util
        spec = importlib.util.spec_from_file_location(
            "confidence_filter_node",
            repo_root / "blocks" / "confidence-filter" / "node.py"
        )
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
...
class TestResultLogger:
    def _make_node(self, monkeypatch, tmp_path, fmt="jsonl"):
        ...
        spec = importlib.util.spec_from_file_location(
            "result_logger_node",
            repo_root / "blocks" / "result-logger" / "node.py"
        )
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
...
class TestConveyorStop:
    def _make_node(self, monkeypatch, **env_vars):
        ...
        spec = importlib.util.spec_from_file_location(
            "conveyor_stop_node",
            repo_root / "blocks" / "conveyor-stop" / "node.py"
        )
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
