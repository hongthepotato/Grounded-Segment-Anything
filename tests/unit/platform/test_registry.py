"""Unit tests for platform/registry.py.
...
def registry_module(blocks_dir, monkeypatch):
    ...
    import platform.registry as reg
...
class TestScanBlocks:
    ...
    def test_scan_finds_valid_blocks(self, registry_module, blocks_dir):
        reg = registry_module
        self._make_block(blocks_dir, "usb-camera", {
            "id": "usb-camera", "version": "1.0.0",
            "display_name": "USB Camera", "category": "sensor",
    ...
    def test_scan_skips_sdk_and_base_dirs(self, registry_module, blocks_dir):
        ...
        for skip_name in ["sdk", "base", "base-cuda"]:
            ...
            with open(d / "block.json", "w") as f:
                json.dump({"id": skip_name, "version": "1.0.0", "display_name": "X", "category": "sensor", "ports": {}}, f)
        blocks = reg.scan_blocks()
        assert blocks == []
    ...
    def test_scan_multiple_blocks(self, registry_module, blocks_dir):
        ...
        for bid in ["cam-a", "cam-b", "detector"]:
            self._make_block(blocks_dir, bid, {
                "id": bid, "version": "1.0.0", "display_name": bid,
                "category": "sensor", "ports": {}
            })
        blocks = reg.scan_blocks()
        assert len(blocks) == 3
    ...
    def test_scan_marks_validation_errors(self, registry_module, blocks_dir, monkeypatch):
        ...
        self._make_block(blocks_dir, "bad-schema", {
            "id": "bad-schema", "version": "1.0.0", "display_name": "X",
            "category": "sensor", "ports": {}
        })
        blocks = reg.scan_blocks()
