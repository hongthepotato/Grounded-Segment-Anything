...
class TestAugmentationRegistry(unittest.TestCase):
    ...
    def test_get_pipeline_basic(self):
        ...
        pipeline = registry.get_pipeline(
            characteristics=["changes_shape"], environment={"lighting": "variable"}, intensity="medium"
        )
    ...
    def test_pipeline_application(self):
        ...
        # Apply augmentation
        result = pipeline(image=self.sample_image, masks=self.sample_masks, bboxes=self.sample_boxes)
    ...
    def test_intensity_levels(self):
        ...
        for intensity in ["low", "medium", "high"]:
            pipeline = registry.get_pipeline(
                characteristics=["changes_shape"], environment={}, intensity=intensity
            )
            self.assertIsNotNone(pipeline)
    ...
    def test_invalid_intensity(self):
        ...
        with self.assertRaises(ValueError):
            registry.get_pipeline(characteristics=["changes_shape"], environment={}, intensity="invalid")
    ...
    def test_multiple_characteristics(self):
        ...
        registry = get_augmentation_registry()
        pipeline = registry.get_pipeline(
            characteristics=["changes_shape", "reflective_surface", "low_contrast"],
            environment={"lighting": "variable", "camera": "fixed"},
            intensity="medium",
        )

        # Apply to sample
        result = pipeline(image=self.sample_image, masks=self.sample_masks)
