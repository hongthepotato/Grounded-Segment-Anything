...
class TestCOCODataset:
    ...
    def test_initialization(self, normalized_data_with_images):
        ...
        dataset = COCODataset(
            coco_data=coco_data, image_path_resolver=path_resolver, return_boxes=True, return_masks=True
        )
...
class TestTeacherDataset:
    ...
    def test_initialization_with_preprocessor(self, normalized_data_with_images):
        ...
        mock_preprocessor.preprocess_batch = Mock(
            return_value={
                "sam": {
        ...
        dataset = TeacherDataset(
            coco_data=coco_data, image_path_resolver=path_resolver, preprocessor=mock_preprocessor
        )
            ...
            },
            {
                ...
                "preprocessed": {
                    "sam": {
                        "image": torch.zeros(3, 1024, 1024),
                        "boxes": np.array(
                            [[100, 100, 50, 50], [200, 200, 60, 60], [300, 300, 70, 70]], dtype=np.float32
                        ),  # 3 boxes
            ...
            },
            {
                ...
                "preprocessed": {
                    "sam": {
                        "image": torch.zeros(3, 1024, 1024),
                        "boxes": np.array(
                            [[100, 100, 50, 50], [200, 200, 60, 60]], dtype=np.float32
                        ),  # 2 boxes
