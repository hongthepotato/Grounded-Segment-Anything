...
def _make_ros2_stubs():
    ...
    for pkg in ("sensor_msgs", "sensor_msgs.msg",
    ...
    mods["sensor_msgs.msg"].Image = lambda: _Msg(
        data=b"", height=0, width=0, encoding="bgr8"
    )
    ...
    mods["vision_msgs.msg"].ObjectHypothesisWithPose = lambda: _Msg(
        hypothesis=_Msg(class_id="", score=0.0)
    )
...
def ros2_stubs():
    ...
    sys.modules.pop(
        "serve.ros2_ws.src.yolo_inference.yolo_inference.node", None
    )
...
class TestEncodingHandling:
    def test_rgb8_converted_to_bgr(self):
        ...
        img_data = np.array([[[255, 0, 0], [0, 255, 0]],
                              [[0, 0, 255], [128, 128, 0]]], dtype=np.uint8)
