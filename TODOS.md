...
### 19. Tighten `augmentation/parameter_system.py` validators to close 5 docstring/enforcement gaps

**What:** Same shape as TODO #18 (api/schemas.py), but for
`augmentation/parameter_system.py`. `tests/unit/augmentation/test_parameter_system.py`
documents each gap as `xfail(strict=True)` (9 individual xfails across 5
categories) — when the corresponding source-code fix lands, the strict-
xfail trips, CI fails, and the developer flips the `@pytest.mark.xfail`
off so the test becomes a regression guard.

**Categories** (each xfail's `reason=` line points at the source line +
recommended fix):

- **`convert_to_numeric` accepts bool** (1 xfail). `isinstance(True, int)`
  is True in Python, so `convert_to_numeric(True)` returns the bool
  `True` (not int 1). Downstream `RangeParameter(False, True)` silently
  creates a `[0.0, 1.0]` range from boolean inputs. Fix: explicit
  `if isinstance(value, bool): raise TypeError(...)` BEFORE the int/float
  check. Source: `parameter_system.py:24`.
- **`convert_to_numeric` rejects `'inf'` / `'nan'` / `'-inf'`** (3 xfails).
  Routing at line 31 sends strings without `.` or `e` to `int()`, which
  rejects these special-value floats. Docstring promises "string
  representations of numbers" are accepted, but `'inf'` falls through.
  Fix: route `'inf'/'-inf'/'nan'` (case-insensitive) to `float()`
  explicitly, OR document them as not-accepted.
- **`RangeParameter(is_integer=True)` silently truncates sub-unit float
  bounds** (3 xfails). E.g., `RangeParameter(0.5, 0.9, is_integer=True)`
  becomes `[0, 0]` via `int()` floor-toward-zero. Real footgun for
  params like `num_holes` where someone passes `(0.5, 2.0)` intending
  `[1, 2]` and gets `[0, 2]`. Fix: round() instead of int(), OR raise on
  non-integer bounds when `is_integer=True`. Source:
  `parameter_system.py:78` (to_albumentations_format) and `:84` (sample).
- **`RangeParameter.integer_range` doesn't catch `OverflowError`** (1
  xfail). `int(float('inf'))` raises OverflowError; the `except` clause
  at line 111 only catches `(TypeError, ValueError)`. Fix: add
  OverflowError to the except tuple.
- **`RangeParameter.is_scalar` uses `==` on floats** (1 xfail).
  `RangeParameter(0.1 + 0.2, 0.3).is_scalar()` returns False due to
  floating-point drift. Fix: `math.isclose()` instead of `==`.
  Source: `parameter_system.py:116`.

**Why deferred from TODO #12:** Same rationale as #18 — the test-
coverage PR (item #12.2) shouldn't bundle behavior changes. Each
tightening is small and independent; can ship as one PR or per-category
mini-PRs depending on review preference.

**Pros:**
- Closes 5 docstring/reality gaps in a low-traffic module.
- 9 xfails turn into 9 regression tests once each gap is closed.
- The bool-as-int fix in particular protects every downstream caller
  (RangeParameter, NestedParameter, AugmentationRule.intensity_ranges)
  from boolean misuse.

**Cons:**
- The float-truncation fix (`is_integer=True` collapse) is a behavior
  change that could affect existing albumentations configs that
  accidentally pass float bounds. Audit `augmentation/characteristic_translator.py`
  before flipping (the `intensity_ranges` data is the main consumer).
- The `is_scalar` `math.isclose` fix has a default tolerance choice
  (rel_tol vs abs_tol) — pick deliberately, not by-default.

**Test surface:** No new tests needed — `tests/unit/augmentation/test_parameter_system.py`
already has them, all currently `xfail(strict=True)`. Per gap fixed,
flip the corresponding `@pytest.mark.xfail` decorator off.

**Context:** Surfaced 2026-04-27 during item #12.2 (P2 unit test roster
— `tests/unit/augmentation/test_parameter_system.py`). Same pattern as
TODO #18 (which captures the parallel set of api/schemas.py gaps
surfaced by item #12.1).

**Depends on / blocked by:** None. Each category is an independent
1-3 line source change. The `is_integer=True` truncation fix wants the
characteristic_translator audit first.
