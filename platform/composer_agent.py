...
from .registry import aliases_compatible, get_block_by_id, ALIAS_COMPAT_MAP
    ...
    return [
        ...
        },
        {
            ...
            "input_schema": {
                "type": "object",
                "properties": {
                    "block_id": {"type": "string", "description": "Block ID from the catalog"}
                },
                "required": ["block_id"],
            },
        },
...
def _execute_tool(tool_name: str, tool_input: Dict, session: Dict, blocks_catalog: List[Dict]) -> Any:
    ...
    if tool_name == "search_blocks":
        ...
        if src_alias is None:
            return {"valid": False, "reason": f"Port '{tool_input['source_port_id']}' not found on {src_block['id']} outputs"}
        if tgt_alias is None:
            return {"valid": False, "reason": f"Port '{tool_input['target_port_id']}' not found on {tgt_block['id']} inputs"}
